...
const SYNTHETIC_SHADOW = getModulePath(
    'synthetic-shadow',
    'iife',
    'es2017',
    'dev'
);
const SYNTHETIC_SHADOW_COMPAT = getModulePath(
    'synthetic-shadow',
    'iife',
    'es5',
    'dev'
...
function getFiles() {
    ...
    return [
        ...
        // TODO: Switch this from .spec
        // createPattern('**/*.js', { watched: false }),

        createPattern('clock/clock.js', { watched: false })
        // createPattern('clock/__tests__/clock.test.js', { watched: false }),
...
module.exports = (config) => {
    config.set({
        ...
        preprocessors: {
            ...
            'clock/clock.js': ['lwc']
            // 'clock/__tests__/clock.test.js': ['lwc'],
    ...
    if (COVERAGE) {
        ...
        config.preprocessors[COMPAT ? LWC_ENGINE_COMPAT : LWC_ENGINE] = [
            'coverage'
        ];
        config.preprocessors[COMPAT ? WIRE_SERVICE_COMPAT : WIRE_SERVICE] = [
            'coverage'
        ];
        config.preprocessors[
            COMPAT ? SYNTHETIC_SHADOW_COMPAT : SYNTHETIC_SHADOW
        ] = ['coverage'];
